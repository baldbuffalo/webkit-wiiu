...
void setCookies(Document* document, const KURL& url, const WTF::String& value)
{
    ...
    if (cookie_domain.isEmpty()) {
        const WTF::String url_host = url.host();
        if (wkcNetCheckCorrectIPAddressPeer(url_host.utf8().data())) {
...
WTF::String cookies(const Document* document, const KURL& url)
{
    ...
    return ResourceHandleManager::sharedInstance()->getCookie(opt_url.host(), opt_url.path(), secure);
}
