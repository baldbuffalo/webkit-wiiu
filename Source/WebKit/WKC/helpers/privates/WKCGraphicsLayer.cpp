...
GraphicsLayerPrivate::GraphicsLayerPrivate(WebCore::GraphicsLayer* parent)
    ...
    , m_marked(false)
{
    m_offscreenLayer[0] = m_offscreenLayer[1] = 0;
...
GraphicsLayerPrivate::~GraphicsLayerPrivate()
{
    ...
    for (int i=0; i<2; i++) {
        if (m_offscreenLayer[i])
            wkcLayerDeletePeer(m_offscreenLayer[i]);
    }
}
...
{
    if (m_offscreenLayer[0] && (ow==m_offscreenSize.fWidth && oh==m_offscreenSize.fHeight))
        return true;

    WKCSize wsize = {ow, oh};
    int layers = isThreeDImageLayer() ? 2 : 1;

    for (int i=0; i<layers; i++) {
        if (m_offscreenLayer[i]) {
            if (!wkcLayerResizePeer(m_offscreenLayer[i], &wsize)) {
                m_offscreenLayer[i] = 0;
                goto fail;
            }
        } else {
            m_offscreenLayer[i] = wkcLayerNewPeer(WKC_LAYER_TYPE_OFFSCREEN, &wsize, m_opticalzoom);
            if (!m_offscreenLayer[i]) {
                goto fail;
            }
        }
...
{
//    ASSERT(alternative_drawcontext || m_offscreenLayer[0] != 0);
    if (!alternative_drawcontext && !m_offscreenLayer[0])
        return;
    ...
    if (layer) {
        int type = wkcLayerGetTypePeer((void *)layer);
    ...
    if (isThreeDImageLayer() && !alternative_drawcontext) {
        displayright = true;
    }
    int i=0;
loop:
    if (alternative_drawcontext) {
        ...
        offscreen_layer = m_offscreenLayer[i];
        dc = wkcLayerGetDrawContextPeer(offscreen_layer);
    }

    if (!dc)
        return;
    WebCore::GraphicsContext gc((PlatformGraphicsContext *)dc);
    gc.save();
#if USE(WKC_CAIRO)
...
{
//    ASSERT(!(alternative_target && alternative_drawcontext));
//    ASSERT((alternative_target && alternative_target->m_offscreenLayer[0]) || alternative_drawcontext || m_offscreenLayer != 0);
    if (alternative_target && alternative_drawcontext)
        return;
    if (!((alternative_target && alternative_target->m_offscreenLayer[0]) || alternative_drawcontext || m_offscreenLayer != 0))
        return;
...
loop:
    ...
    if (alternative_target) {
        offscreen_layer = alternative_target->m_offscreenLayer[i];
        dc = wkcLayerGetDrawContextPeer(offscreen_layer);
    ...
    gc.clip(real_rect);
#if 0
    const WKCFloatRect freal_rect = { rect.fX - offset.fX, rect.fY - offset.fY, rect.fWidth, rect.fHeight };
    gc.clearRect(freal_rect);
#endif
    gc.translate(-offset.fX, -offset.fY);
    if (opacity < 1.0f)
        gc.beginTransparencyLayer(opacity);
    m_webcore->paintGraphicsLayerContents(gc, rect);
    if (opacity < 1.0f)
...
int
GraphicsLayerPrivate::side() const
{
    if (isThreeDImageLayer())
        return WebCore::ELeftSide;

    if (m_webcore->isThreeDBlockLayer())
        return m_webcore->threeDBlockLayerSide();

    return WebCore::EBothSide;
}
...
GraphicsLayerPrivate::setContentsToImage(WebCore::Image* img)
{
    ...
    void* dc = wkcLayerGetDrawContextPeer(m_offscreenLayer[0]);
    if (!dc)
        return;
    WebCore::GraphicsContext gc((PlatformGraphicsContext *)dc);
    gc.save();
    ...
    gc.restore();
    wkcLayerReleaseDrawContextPeer(m_offscreenLayer[0], dc);
...
void*
GraphicsLayerPrivate::platformLayer(int i) const
{
    if (i==WKC::GraphicsLayer::ERightSide)
        i = 1;
    else
        i = 0;
    return reinterpret_cast<void*>(m_offscreenLayer[i]);
}
...
void
GraphicsLayerPrivate::syncCompositingState(const WKCFloatRect& fr)
{
...
bool
GraphicsLayerPrivate::isThreeDImageLayer() const
{
    return m_webcore->isThreeDImageLayer();
}
...
{
    if (m_offscreenLayer[0])
        wkcLayerDidAttachToTreePeer(m_offscreenLayer[0]);
    if (isThreeDImageLayer() && m_offscreenLayer[1])
        wkcLayerDidAttachToTreePeer(m_offscreenLayer[1]);
}
...
{
    if (m_offscreenLayer[0])
        wkcLayerDidDetachFromTreePeer(m_offscreenLayer[0]);
    if (isThreeDImageLayer() && m_offscreenLayer[1])
        wkcLayerDidDetachFromTreePeer(m_offscreenLayer[1]);
}
...
GraphicsLayerPrivate::setOpticalZoom(float zoom)
{
    ...
    if (!wkcLayerSetOpticalZoomPeer(m_offscreenLayer[0], zoom)) {
        m_offscreenLayer[0] = 0;
    ...
    if (isThreeDImageLayer() && m_offscreenLayer[1]) {
        if (!wkcLayerSetOpticalZoomPeer(m_offscreenLayer[1], zoom)) {
            m_offscreenLayer[1] = 0;
            wkcMemoryNotifyMemoryAllocationErrorPeer(m_offscreenSize.fWidth * m_offscreenSize.fHeight * 4 * m_opticalzoom, WKC_MEMORYALLOC_TYPE_LAYER); // The size may not be accurate, since the error caused in peer.
            disposeOffscreen();
            return;
        }
...
GraphicsLayerPrivate::updateFixedPosition()
{
    ...
    WebCore::RenderLayer* renderLayer = client ? client->owningLayer() : 0;
    if (!renderLayer)
        return;
    ...
    if (m_webcore->client()) {
        if (modified)
            m_webcore->client()->notifySyncRequired(m_webcore);
    }
}
...
bool
GraphicsLayerPrivate::backgroundColorSet() const
{
    return m_webcore->backgroundColorSet();
}
...
void 
GraphicsLayerPrivate::setOffscreenBitmap( void* bitmap )
{
    if (m_offscreenLayer[0])
        wkcLayerSetOffscreenBitmapPeer( m_offscreenLayer[0], bitmap );
}
#endif
...
void*
GraphicsLayer::platformLayer(int i) const
{
    return m_private.platformLayer(i);
}
...
bool
GraphicsLayer::isThreeDImageLayer() const
{
    return m_private.isThreeDImageLayer();
}

int
GraphicsLayer::side() const
{
    return m_private.side();
}
...
bool
GraphicsLayer::backgroundColorSet() const
{
    return m_private.backgroundColorSet();
}
...
#ifdef WKC_CUSTOMER_PATCH_0304674
void 
GraphicsLayer::setOffscreenBitmap( void* bitmap )
{
    m_private.setOffscreenBitmap( bitmap );
}
#endif
...
bool
GraphicsLayerPrivate::addDirtyRect(const WKCFloatRect&)
{
    return true;
}
...
void
GraphicsLayer::dirtyRects(int, WKCFloatRect&) const
{
}
