...
with open(out_file, 'w') as f:
    ...
    f.write('template<> struct DefaultHash<WebCore::CSSPropertyID> : IntHash<unsigned> { };\n')
    f.write('template<> struct HashTraits<WebCore::CSSPropertyID> : GenericHashTraits<WebCore::CSSPropertyID> {\n')
    f.write('    static const bool emptyValueIsZero = true;\n')
    f.write('    static void constructDeletedValue(WebCore::CSSPropertyID& slot) { slot = static_cast<WebCore::CSSPropertyID>(std::numeric_limits<uint16_t>::max()); }\n')
    f.write('    static bool isDeletedValue(WebCore::CSSPropertyID value) { return static_cast<uint16_t>(value) == std::numeric_limits<uint16_t>::max(); }\n')
    f.write('};\n')
    f.write('} // namespace WTF\n')
