...
*/
WKC_PEER_API bool wkcTimerInitializePeer(bool(*in_proc)(unsigned int, void*));
/**
...
@param "in_timer" Pointer to timer object
@param "in_timeout" Time period until timeout (ms)
...
The existing implementation can be used for this function without change.
*/
WKC_PEER_API bool wkcTimerStartOneShotPeer(void* in_timer, unsigned int in_timeout, wkcTimeoutProc in_proc, void* in_data);
/**
...
The existing implementation can be used for this function without change.
*/
WKC_PEER_API bool wkcTimerStartPeriodicPeer(void* in_timer, unsigned int in_timeout, wkcTimeoutProc in_proc, void* in_data);
/**
@brief Stops timer
@param "in_timer" Pointer to timer object
@details
...
// debug print peer
/**
@brief Initializes debug output function
@retval "!= false" Succeeded
@retval "== false" Failed
@details
Performs the initialization process of debug output function. Implementing it is required only if needed by platform.
*/
...
/**
@brief Forcibly terminates threads in the Thread Peer layer
@details
Forcibly terminates the threads in the Thread Peer layer.
Do not call wkcThreadForceTerminatePeer() if you call this function.
*/
WKC_PEER_API void wkcThreadForceTerminateThreadsPeer(void);
/**
@brief Forcibly terminates resources other than threds in the Thread Peer layer
@details
Forcibly terminates resources other than threds in the Thread Peer layer.
Must be called after wkcThreadForceTerminateThreadsPeer().
Do not call wkcThreadForceTerminatePeer() if you call this function.
*/
WKC_PEER_API void wkcThreadForceTerminateExceptThreadsPeer(void);
...
Generates thread.
*/
WKC_PEER_API void* wkcThreadCreatePeer(wkcThreadProc in_proc, void* in_data);
WKC_PEER_API void* wkcThreadCreateForMultiCorePeer(wkcThreadProc in_proc, void* in_data);
/**
...
WKC_PEER_API void* wkcThreadGetMainThreadPeer(void);
// add for peer unittest...
WKC_PEER_API bool wkcThreadIsSuspendedPeer(void* in_thread);
WKC_PEER_API void wkcThreadSetMutexPeer(void* in_thread, void* in_mutex);
WKC_PEER_API void* wkcThreadGetMutexPeer(void* in_thread);
WKC_PEER_API void wkcThreadSetCondPeer(void* in_thread, void* in_cond);
WKC_PEER_API void* wkcThreadGetCondPeer(void* in_thread);
/**
...
@param "in_cond" Pointer to condition variable object
@param "in_mutex" Pointer to mutex object
@param "in_offsettime_ms" Maximum waiting period (ms)
@retval "!= false" Succeeded
@retval "== false" Failed
@details
...
*/
WKC_PEER_API bool wkcCondTimedWaitPeer(void* in_cond, void* in_mutex, unsigned int in_offsettime_ms);
/**
...
- The behavior must be the same as the pthread_cond_timedwait() function.
*/
WKC_PEER_API bool wkcCondTimedWaitLightPeer(void* in_cond, WKCLightMutex* in_mutex, unsigned int in_offsettime_ms);
...
Calls the callback function, WKCFileProcs::fFOpenProc, set by wkcFileCallbackSetPeer().
*/
WKC_PEER_API int        wkcFileFSyncPeer(void *in_fd);
/**
...
Calls the callback function, WKCFileProcs::fFOpenProc, set by wkcFileCallbackSetPeer().
*/
WKC_PEER_API bool       wkcFileUnlinkPeer(const char* in_path);
/**
@brief Gets file information
...
WKC_PEER_API void*      wkcFileOpenDirectoryPeer(int in_usage, const char* in_path, const char *in_filter);
WKC_PEER_API void       wkcFileCloseDirectoryPeer(void* in_dir);
WKC_PEER_API int        wkcFileReadDirectoryPeer(void* in_dir, char* out_entryname, int in_maxentrynamelen);
...
@param in_mask Mask
@param in_maskrowbytes Row bytes of mask
@param in_size Size of image
@return None
*/
WKC_PEER_API void wkcPasteboardWriteImagePeer(int in_type, void* in_bitmap, int in_rowbytes, void* in_mask, int in_maskrowbytes, const WKCSize* in_size);
...
#ifdef NDEBUG
#define PEER_DP(a) ((void)0)
#define PEER_ENTER(a) ((void)0)
#else
WKC_PEER_API void peerDebugPrintf(const char*,...);
#define PEER_DP(a) peerDebugPrintf a
#define PEER_ENTER(a) peerDebugPrintf a
#endif
