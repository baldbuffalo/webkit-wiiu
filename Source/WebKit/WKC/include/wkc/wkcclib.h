...
// If the target platform doesn't have sys/XXX.h, create an alternative header.
#if defined(__ghs__)
#include <sys/types.h>
...
@return Allocated memory. The memory must be freed by free_aligned().
@attention
Do not change this function since it operates with the current implementation. 
*/
WKC_PEER_API void *malloc_aligned(size_t size, size_t align_size);
/**
@brief Free memory allocated by malloc_aligned()
@param ptr Pointer to free
@return None
@attention
Do not change this function since it operates with the current implementation. 
*/
WKC_PEER_API void free_aligned(void *ptr);
...
WKC_PEER_API void   *wkc_opendirectory(const char *path, const char *filter);
WKC_PEER_API void   wkc_closedirectory(void *dir);
WKC_PEER_API int    wkc_readdirectory(void *dir, char *entryname, int maxentrylen);

// add for peer unittest
WKC_PEER_API int wkc_unlink(const char* pathname);
...
WKC_PEER_API int     wkc_rename(const char *oldpath, const char *newpath);
...
*/
#if defined(__ghs__)
// TODO: Use the API using off_t below and remove this IF block.
...
/**
@brief POSIX gettimeofday() function with long long tv_sec
@param tv Conforms to POSIX gettimeofday() but tv_sec should be bigger than 32-bit
@param tz Conforms to POSIX gettimeofday()
@return Conforms to POSIX gettimeofday()
@attention
The behavior must be the same as the POSIX gettimeofday() function.
...
WKC_PEER_API int wkc_gettimeofday64(struct timeval64 *tv, struct timezone *tz);
/**
...
#if defined(WKC_USE_NETWORKTIME)
WKC_PEER_API void wkc_setValidNetworktime(bool valid);
#endif // defined(WKC_USE_NETWORKTIME)
#endif
