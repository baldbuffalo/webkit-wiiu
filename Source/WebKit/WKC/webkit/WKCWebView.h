...
#define WKC_USE_CUSTOM_BACKFORWARD_LIST
...
@param memory_size Size of heap memory for browser engine (bytes)
...
public:
    ...
    */
    ...
    /**
       ...
       @param needsLayout Setting for invoking layout
       - != false Invoke layout.
       - == false Do not invoke layout.
       @retval "!= false" Succeeded
       @retval "== false" Failed
    */
    ...
    */
    ...
    /**
       @brief Requests performe animations requested by ChromeClientIf::scheduleAnimation().
       @return None
       @details
       Requests performe animations requested by ChromeClientIf::scheduleAnimation().
    */
    ...
    */
    void notifyFocusOut();
    /**
       @brief Find nearest focusable node in the direction
       @param direction Direction to search node
       @param specificRect Rectangle to search node
       @retval "!=NULL" Nearest focusable node
       @retval "==NULL" Not found
       @details
       Find nearest focusable node in the direction,
    */
    WKC::Node* findFocusableNode(const WKC::FocusDirection direction, const WKCRect* specificRect = 0);
    /**
       @brief Find nearest focusable node in the direction
       @param direction Direction to search node
       @param rect Rectangle to search node
       @param enableConatiner Allow container node is focusable
       @retval "!=NULL" Nearest focusable node
       @retval "==NULL" Not found
       @details
       Find nearest focusable node in the direction,
    ...
    */
    ...
    /**
      @brief Notify gamepad event
      @param index index of gamepads
    ...
    */
	...
	void selectionStartEnd(WKCPoint &startTop, WKCPoint &startBottom, WKCPoint &endTop, WKCPoint &endBottom);
	void selectWordAt(const WKCPoint &target);
	void changeSelectionAt(bool base, const WKCPoint &target);
	void changeSelectionByOne(bool base, bool left);
    ...
    */
    ...
    /**
       @brief Clears cache
       @param clearhttpcache
       - != false clear httpcache
       - == false do not clear httpcache
       @return None
       @details
       Clears the cache.
    */
    ...
    */
    ...
    // icon database
    /**
       @brief Sets the folder to store icon database
       @param in_folder Folder
       @return None
       @details
       Sets the folder to store icon database.
    */
    static void setIconDatabaseFolder(const char* in_folder);
    /**
       @brief Sets icon database on memory
       @return None
       @details
       Sets icon database is located on memory.
    */
    static void setIconDatabaseOnMemory();
    /**
       @brief Clears icon database
       @return None
       @details
       Clears icon database.
    */
    ...
    */
    ...
    /**
       @brief Gets number of RSS Link elements contained in displayed page
       @return >=0 Number of RSS Link elements
       @details
       This function must be performed after notification of FrameLoaderClientWKC::dispatchDidFinishDocumentLoad is given.
    */
    unsigned int getRSSLinkNum();
    /**
       @brief Gets RSS Link element information contained in displayed page
       @param info Pointer to WKC::WKCRSSLinkInfo array
       @param info_len Length of WKC::WKCRSSLinkInfo array
       @return Number of RSS Link elements copied
       @details
       Copies RSS Link element information to info.@n
       The length of the WKC::WKCRSSLinkInfo array is the return value of WKCWebView::getRSSLinkNum(), and the memory range for WKC::WKCRSSLinkInfo must be allocated by the caller side.
    */
    unsigned int getRSSLinkInfo(WKCRSSLinkInfo* info, unsigned int info_len);
    /**
       @brief Gets pointer to WKC::Node object of element that has focus
       @return Pointer to WKC::Node object
       @details
       This function can get various information about an element, from the WKC::Node object.@n
       - Element name WKC::Node::nodeName()@n
       Equivalent to the element name in W3C.
       - Attributes WKC::Node::attributes()@n
       Equivalent to attributes name in W3C.
       - Coordinates and size WKC::Node::renderer()->absoluteBoundingBoxRect()
       @attention
       If an event, etc. occurs during the period after WKCWebView::getFocusedNode is called and before information is obtained from WKC::Node, the WKC::Node information may become unusable.@n
       In this case, the information must be obtained from WKC::Node after calling WKCWebView::getFocusedNode again.
    */
    WKC::Node* getFocusedNode();
    /**
       @brief Gets pointer to WKC::Node object of element below the pointer using specified coordinates
       ...
       @attention
       If an event, etc. occurs during the period after WKCWebView::getNodeFromPoint is called and before information is obtained from WKC::Node, the WKC::Node information may become unusable.@n
       In this case, the information must be obtained from WKC::Node after calling WKCWebView::getNodeFromPoint again.
    */
    ...
    */
    ...
    /**
       @brief Enables / disables to maintain back-forward list
       @param flag Enables / disables
       @return None
       @attention
       This API cannot be called if the application uses own implementation of BackForwardClient.
    */
    void setMaintainsBackForwardList(bool flag);
    /**
       @brief Adds transition history information to browser engine
       @param uri URI of transition destination
       @param title Title of transition destination
       @param scrollPoint Scroll point
       @return None
       @details
       When starting the browser, the application can add transition history information to the browser engine by calling this API.
       @attention
       Either ASCII-only or UTF-8 C strings must be used for uri.@n
       UTF-16 strings must be used for title. 
       This API cannot be called if the application uses own implementation of BackForwardClient. WKC::HistoryItem::create can be used for creating a new HistoryItem if you want.
    */
    void addHistoryItem(const char* uri, const unsigned short* title, const WKCPoint* scrollPoint = 0);
    /**
       @brief Gets length of browser engine transition history list
       @return Length of transition history list
       @attention
       This API cannot be called if the application uses own implementation of BackForwardClient.
    */
    unsigned int getHistoryLength();
    /**
       @brief Gets index value of displayed page in browser engine transition history list
       @param index Index value to get
       @retval "!= false" Succeeded in getting index (index = 0 to WKC::WKCWebView::getHistoryLength()-1)
       @retval "== false" Failed to get index (displayed page does not exist, index = undetermined value)
       @details
       During the transition history restoration process, calling WKC::WKCWebView::gotoHistoryItemByIndex with the index obtained by this API as an argument will restore the view in the displayed page.
       @attention
       This API cannot be called if the application uses own implementation of BackForwardClient.
    */
    bool getHistoryCurrentIndex(unsigned int& index);
    /**
       @brief Gets index value of specified history item
       @param item History item
       @param index Index value to get
       @retval "!= false" Succeeded in getting index (index = 0 to WKC::WKCWebView::getHistoryLength()-1)
       @retval "== false" Failed to get index (index = undetermined value)
       @details
       Calling this API for the history item given in the FrameLoaderClientWKC::shouldGoToHistoryItem argument allows you to get the index value of the target history item.
       @attention
       This API cannot be called if the application uses own implementation of BackForwardClient.
    */
    bool getHistoryIndexByItem(WKC::HistoryItem* item, unsigned int& index);
    /**
       @brief Deletes specific item from browser engine transition history list
       @param index Index value of item to delete (0 to WKC::WKCWebView::getHistoryLength()-1)
       @return None
       @attention
       This API cannot be called if the application uses own implementation of BackForwardClient.
    */
    ...
    /**
       @brief Gets information about specified item in browser engine transition history list
       @param index Index value of item to get (0 to WKC::WKCWebView::getHistoryLength()-1)
       @param uri Pointer to buffer that outputs item uri (application must allocate memory)
       @param uriLen Length of uri to get (not including terminal NULL)
       @param title Pointer to buffer that outputs item title (application must allocate memory)
       @param titleLen Length of title to get (not including terminal NULL)
       @retval "!= false" Succeeded in getting information
       @retval "== false" Failed to get information
       @details
       If the application specified 0 for uri or title, this API returns uriLen and titleLen. The application must allocate buffer area for writing the uri or title from the length of each bit of information, and then call this API again.@n
       And, to write the NULL terminus also in this API, it must add the NULL terminal size to the length of each buffer (+1) and allocate the memory.
       @attention
       This API cannot be called if the application uses own implementation of BackForwardClient.
    */
    ...
    /**
       @brief Moves to specified item in browser engine transition history
       @param index Index value of item to move to (0 to WKC::WKCWebView::getHistoryLength()-1)
       @return None
       @details
       Displays the previously viewed item (index value must be obtained from WKC::WKCWebView::getHistoryCurrentIndex) in the transition history restoration process.
       @attention
       This API must be called after WKC::WKCWebView::addHistoryItem is called. And, do not use this API for transition history processes other than the restoration process (use WKC::WKCWebView::goBackOrForward).
       This API cannot be called if the application uses own implementation of BackForwardClient.
    */
    ...
    enum {
        ...
        EInternalColorFormat8888or565, // shrink to 565 if no alpha color used
    ...
    /**
       ...
       @param fmt Format
       @details
       For fmt, specify WKC::WKCWebView::EInternalColorFormat8888 or WKC::WKCWebView::EInternalColorFormat5515withMask.
    */
    ...
    */
    ...
    /**
       @brief Sets width to wrap inline text
       @param width width to wrap in pixel
    */
    ...
    */
    ...
    // JS Heap
    static size_t jsHeapAllocatedBlockCount(); /* The number of JSC::MarkedBlocks which are allocated by JSC::Heap. */
    static size_t jsHeapAllocatedCellSize(); /* The total bytes of JSC::JSCells which are allocated by JS engine using JSC::Heap::allocate. */
    static size_t jsHeapWaterMark();
    ...
    */
    ...
    // page visibility
    /** @brief Page visibility states */
    enum {
        /** @brief Page is visible */
        PageVisibilityStateVisible,
        /** @brief Page is hidden */
        PageVisibilityStateHidden,
        /** @brief Page is in pre-render state */
        PageVisibilityStatePrerender,
        /** @brief Page is in preview mode */
        PageVisibilityStatePreview,
        /** @brief number of states */
        PageVisibilityStates
    };
    /**
       @brief Sets page visibility state
       @param state Visibility state
       @param isInitialState Is initial state
       @retval None
       @details
       Sets page visibility state.
    */
    ...
    */
    ...
    /**
       @brief Returns whether or not the document is in fullscreen mode.
       @retval "!= false" In fullscreen mode
       @retval "== false" Not in fullscreen mode
     */
