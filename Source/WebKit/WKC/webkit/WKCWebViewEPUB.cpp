...
WKC_DEFINE_GLOBAL_PTR(WTF::Vector<OpenedEPUB*>*, gOpenedEPUBs, 0);
...
WKC_DEFINE_GLOBAL_PTR(WTF::Vector<OpenedZIPItem*>*, gOpenedZIPItems, 0);
...
clearNav(WKCEPUBDataNav* self)
{
    ...
    for (size_t i=0; i<count; i++) {
        ...
        if (item) {
            if (item->fToc)
                wkc_free((void *)item->fToc);
            if (item->fTitle)
                wkc_free((void *)item->fTitle);
            if (item->fUri)
                wkc_free((void *)item->fUri);
            if (item->fType)
                wkc_free((void *)item->fType);
            wkc_free(item);
        }
    }
    if (self->fItems)
        wkc_free(self->fItems);
    ...
    if (self->fChildren)
        wkc_free(self->fChildren);

    if (self->fType)
        wkc_free((void *)self->fType);
    ...
    if (m_EPUBData.fSpines) {
        for(int i=0; i<m_EPUBData.fSpineItems; i++) {
            if (m_EPUBData.fSpines[i].fSpine)
                wkc_free((void *)m_EPUBData.fSpines[i].fSpine);
            if (m_EPUBData.fSpines[i].fProperties)
                wkc_free((void *)m_EPUBData.fSpines[i].fProperties);
        }
        wkc_free(m_EPUBData.fSpines);
    }
    m_EPUBData.fSpines = 0;
    m_EPUBData.fSpineItems = 0;
        ...
        if (m_EPUBData.fTitle.fStrings) {
            for (int i=0; i<m_EPUBData.fTitle.fItems; i++)
                if (m_EPUBData.fTitle.fStrings[i])
                    wkc_free((void *)m_EPUBData.fTitle.fStrings[i]);
            wkc_free((void *)m_EPUBData.fTitle.fStrings);
        }
        ...
        if (m_EPUBData.fCreator.fStrings) {
            for (int i=0; i<m_EPUBData.fCreator.fItems; i++)
                if (m_EPUBData.fCreator.fStrings[i])
                    wkc_free((void *)m_EPUBData.fCreator.fStrings[i]);
            wkc_free((void *)m_EPUBData.fCreator.fStrings);
        }
        ...
        if (m_EPUBData.fContributor.fStrings) {
            for (int i=0; i<m_EPUBData.fContributor.fItems; i++)
                if (m_EPUBData.fContributor.fStrings[i])
                    wkc_free((void *)m_EPUBData.fContributor.fStrings[i]);
            wkc_free((void *)m_EPUBData.fContributor.fStrings);
        }
        ...
        if (m_EPUBData.fLanguage.fStrings) {
            for (int i=0; i<m_EPUBData.fLanguage.fItems; i++)
                if (m_EPUBData.fLanguage.fStrings[i])
                    wkc_free((void *)m_EPUBData.fLanguage.fStrings[i]);
            wkc_free((void *)m_EPUBData.fLanguage.fStrings);
        }
        ...
        if (m_EPUBData.fRights.fStrings) {
            for (int i=0; i<m_EPUBData.fRights.fItems; i++)
                if (m_EPUBData.fRights.fStrings[i])
                    wkc_free((void *)m_EPUBData.fRights.fStrings[i]);
            wkc_free((void *)m_EPUBData.fRights.fStrings);
        }
        ...
        if (m_EPUBData.fPublisher.fStrings) {
            for (int i=0; i<m_EPUBData.fPublisher.fItems; i++)
                if (m_EPUBData.fPublisher.fStrings[i])
                    wkc_free((void *)m_EPUBData.fPublisher.fStrings[i]);
            wkc_free(m_EPUBData.fPublisher.fStrings);
        }

    if (m_EPUBData.fDate)
        wkc_free((void *)m_EPUBData.fDate);
    if (m_EPUBData.fModified)
        wkc_free((void *)m_EPUBData.fModified);
    if (m_EPUBData.fNavUri)
        wkc_free((void *)m_EPUBData.fNavUri);
...
WKCWebView::EPUB::openEPUB(const char* in_path, bool parseTOC)
{
    ...
    OpenedEPUB* item = new OpenedEPUB();
...
allocEPUBStringArray(const WTF::Vector<WTF::String> items, WKCEPUBStringArray& dest)
{
    ...
    dest.fStrings = (const unsigned char **)wkc_malloc(sizeof(const unsigned char *)*dest.fItems);
    if (!dest.fStrings)
        return false;

    for (int i=0; i<dest.fItems; i++) {
        dest.fStrings[i] = (const unsigned char *)wkc_strdup(items[i].utf8().data());
        if (!dest.fStrings[i])
            return false;
    }
    return true;
                ...
                if (isTargetElement(n2, 0, "rootfiles")) {
                    for (xmlNodePtr n3 = n2->children; n3; n3 = n3->next) {
                        if (isTargetElement(n3, 0, "rootfile")) {
                            ...
                            if (fullpath) {
                                contentpath = wkc_strdup(fullpath);
                                goto found;
                            }
                        }
                    }
...
found:
    ...
    if (strchr(contentpath, '/') || strchr(contentpath, '\\')) {
        basepath = wkc_strdup(contentpath);
        if (!basepath)
            goto error_end;
        {
    ...
    rootpath = (char *)wkc_malloc(1 + strlen(m_path) + 1 + strlen(basepath) + 1);
    if (!rootpath)
        goto error_end;
    rootpath[0] = 0;
                    ...
                    for (xmlNodePtr n3 = n2->children; n3; n3 = n3->next) {
                        if (isTargetElement(n3, 0, "date")) {
                            if (item = getContent(n3->children)) {
                                if (m_EPUBData.fDate)
                                    wkc_free((void *)m_EPUBData.fDate);
                                m_EPUBData.fDate = (const unsigned char *)wkc_strdup((char *)item);
                            }
                            ...
                            if (prop && strcasecmp((const char *)prop, "dcterms:modified")==0) {
                                if (item = getContent(n3->children)) {
                                    if (m_EPUBData.fModified)
                                        wkc_free((void *)m_EPUBData.fModified);
                                    m_EPUBData.fModified  = (const unsigned char *)wkc_strdup((char *)item);
                                }
                            }
                        }
                    }
                        ...
                        if (isTargetElement(n3, 0, "item")) {
                            ...
                            if (href) {
                                ...
                                if (properties && strcasecmp((const char *)properties, "nav")==0) {
                                    if (navuri)
                                        wkc_free((void *)navuri);
                                    navuri = wkc_strdup((const char *)href);
                                }
                            }
                        }
                    }
    ...
    m_EPUBData.fSpines = (WKCEPUBDataSpine *)wkc_malloc(sizeof(WKCEPUBDataSpine)*spines.size());
    if (!m_EPUBData.fSpines)
        goto error_end;
    for (int i=0; i<spines.size(); i++) {
        ...
        if (href.length()) {
            ...
            m_EPUBData.fSpines[i].fSpine = (unsigned char *)wkc_strdup(s.utf8().data());
            if (!m_EPUBData.fSpines[i].fSpine)
                goto error_end;
            if (properties.length()) {
                m_EPUBData.fSpines[i].fProperties = (unsigned char *)wkc_strdup(properties.utf8().data());
                if (!m_EPUBData.fSpines[i].fProperties)
                    goto error_end;
            }
            else
    ...
    if (navuri && parseTOC) {
        ...
        char* tocpath = wkc_strdup(rootpath);
        if (!tocpath)
            goto error_end;
        ...
        if (p) {
            ...
            WTF::String tp = rp + '/' + np;
            tocpath = wkc_strdup(tp.utf8().data());
            if (!tocpath)
                goto error_end;
        }
        parseToc(m_zip, m_EPUBData, path.utf8().data(), tocpath);
        WTF::String s = WTF::String::format("file://%s/%s", rootpath, navuri);
        m_EPUBData.fNavUri = (unsigned char *)wkc_strdup(s.utf8().data());
        wkc_free(navuri);
        wkc_free(tocpath);
    }

    wkc_free(contentpath);
    wkc_free(rootpath);
    wkc_free(basepath);
...
createNavTree(const EPUBDataNav* nav, const char* rootpath)
{
    ...
    WKCEPUBDataNav* navs = (WKCEPUBDataNav *)wkc_malloc(sizeof(WKCEPUBDataNav));
    if (!navs)
        return 0;
    ...
    if (count) {
        ...
        navs->fItems = (WKCEPUBDataTocItem **)wkc_malloc(sizeof(WKCEPUBDataTocItem)*count);
        if (!navs->fItems)
            goto error_end;
        for (size_t i=0; i<count; i++) {
            WKCEPUBDataTocItem* item = (WKCEPUBDataTocItem *)wkc_malloc(sizeof(WKCEPUBDataTocItem));
            item->fIndex = nav->items[i].index;
            item->fToc = (const unsigned char *)wkc_strdup(nav->items[i].toc.utf8().data());
            item->fTitle = (const unsigned char *)wkc_strdup(nav->items[i].title.utf8().data());
            item->fType = (const unsigned char *)wkc_strdup(nav->items[i].type.utf8().data());
            WTF::String href = WTF::String::format("file://%s/%s", rootpath, nav->items[i].href.utf8().data());
            item->fUri = (const unsigned char *)wkc_strdup(href.utf8().data());
            if (!item->fToc || !item->fUri || !item->fTitle)
                goto error_end;
            navs->fItems[i] = item;
        }
    }
    ...
    if (count) {
        ...
        navs->fChildren = (WKCEPUBDataNav**)wkc_malloc(sizeof(WKCEPUBDataNav*)*count);
        if (!navs->fChildren)
            goto error_end;
        for (size_t i=0; i<count; i++) {
...
parseToc(void* zipa, WKCEPUBData& data, const char* tocpath, const char* rootpath)
{
    ...
    data.fNavs = (WKCEPUBDataNav **)wkc_malloc(sizeof(WKCEPUBDataNav*)*navitems);
    if (!data.fNavs)
        goto error_end;
    for (int i=0; i<navitems; i++) {
        data.fNavs[i] = createNavTree(navs[i], rootpath);
        if (navtypes[i].length())
            data.fNavs[i]->fType = (const unsigned char *)wkc_strdup(navtypes[i].utf8().data());
        else
...
EPUBFOpen(int in_usage, const char* in_path, const char* in_mode)
{
    for (int i=0; i<gOpenedEPUBs->size(); i++) {
        ...
        if ((strlen(in_path) > strlen(item->m_path)) &&
            ...
            char* path = wkc_strdup(in_path + strlen(item->m_path)+1);
            if (!path)
                return 0;
            int len = strlen(path);
            for (int j=0; j<len; j++) {
...
EPUBStat(const char* in_filename, struct stat* out_buf)
{
    for (int i=0; i<gOpenedEPUBs->size(); i++) {
        ...
        if ((strlen(in_filename) > strlen(item->m_path)) &&
            ...
            char* path = wkc_strdup(in_filename + strlen(item->m_path)+1);
            if (!path)
                return -1;
            int len = strlen(path);
            for (int j=0; j<len; j++) {
